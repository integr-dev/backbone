...
/**
 * A command argument that parses a long value.
 *
 * This argument accepts any valid long as input.
 ...
 * @since 1.0.0
 */
fun longArgument(name: String, description: String): Argument<Long> {
    return LongArgument(name, description)
}
...
class LongArgument(name: String, description: String) : Argument<Long>(name, description) {
    override fun getCompletions(current: ArgumentInput): CompletionResult {
        ...
        return CompletionResult(if (arg.text.isBlank()) mutableListOf("<$name:long>") else mutableListOf(), arg.end)
    }
